...
若终端出现 **`Failed to open database` / `invalid digit found in string`**，这是 **Turbopack 本地缓存**损坏，与 PostgreSQL 无关。可执行 `npm run clean:next` 后重新 `npm run dev`，或改用 **`npm run dev:webpack`** 走 Webpack 开发。
